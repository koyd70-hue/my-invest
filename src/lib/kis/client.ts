import { KisPriceResult, KisMonthPriceResult, Market, StockSearchResult } from '@/types';

export async function searchKisStocks(query: string): Promise<StockSearchResult[]> {
  const res = await fetch('/api/kis/search', {
    method: 'POST',
